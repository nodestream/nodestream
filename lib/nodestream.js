...
class Nodestream {
  ...
  upload(file, options) {
    ...
    .then(destination => {
      ...
      for (const transformer of transforms)
        result[transformer.constructor.namespace] = transformer.results()
  ...
  addTransform(direction, transformer, options) {
    ...
    if (typeof transformer.namespace !== 'string')
      throw new ReferenceError(`Transformer ${transformer.name} does not declare namespace`)
