  ...
  upload(file, options) {
    ...
    file = this[scope].transforms.upload.reduce(
      (upstream, transform) => {
        ...
        // TODO: We should also handle these forwarded errors in the .upload() function and not
        // require the adapter to handle them
        upstream.on('error', err => transformed.emit('error', err))

        return transformed
      }, file)

    // Regardless of what kind of Promise implementation the adapter returns, our caller MUST
    // receive a standard ES 2015 Promise instance
    return Promise.resolve(adapter.upload(file, options))
    .then(location => {
